...
while getopts n:g:r:s:p:d:o:a: option
do
case "${option}"
in
n) WILD_TYPE=${OPTARG};;
...
# Clean and remake directories if remapping
...
# Select appropriate genomes data

if [ "$GENOME" = "Saccharomyces_cerevisiae" ]
then

	GENOME=/Main/MutantHuntWGS/Genomes/S_cerevisiae/genome \
	GENOME_FASTA=/Main/MutantHuntWGS/Genomes/S_cerevisiae/genome.fa \
	SIFT_GENOME=/Main//R64-1-1.23

fi


if [ "$GENOME" = "Schizosaccharomyces_pombe" ]
then

	GENOME=/Main/MutantHuntWGS/Genomes/S_pombe/genome \
	GENOME_FASTA=/Main/MutantHuntWGS/Genomes/S_pombe/genome.fa \
	SIFT_GENOME=/Main/ASM294v2.22

fi


if [ "$GENOME" = "Arabidopsis_thaliana" ]
then

	GENOME=/Main/MutantHuntWGS/Genomes/A_thaliana/genome \
	GENOME_FASTA=/Main/MutantHuntWGS/Genomes/A_thaliana/genome.fa \
	SIFT_GENOME=/Main/TAIR10.23

fi

if [ "$GENOME" = "Caenorhabditis_elegans" ]
then

	GENOME=/Main/MutantHuntWGS/Genomes/C_elegans/genome \
	GENOME_FASTA=/Main/MutantHuntWGS/Genomes/C_elegans/genome.fa \
	SIFT_GENOME=/Main/WBcel235.83

fi

if [ "$GENOME" = "Drosophila_melanogaster" ]
then

	GENOME=/Main/MutantHuntWGS/Genomes/D_melanogaster/genome \
	GENOME_FASTA=/Main/MutantHuntWGS/Genomes/D_melanogaster/genome.fa \
	SIFT_GENOME=/Main/BDGP6.83

fi

if [ "$GENOME" = "Danio_rerio" ]
then

	GENOME=/Main/MutantHuntWGS/Genomes/D_rerio/genome \
	GENOME_FASTA=/Main/MutantHuntWGS/Genomes/D_rerio/genome.fa \
	SIFT_GENOME=/Main/GRCz10.83

fi

if [ "$GENOME" = "Mus_musculus" ]
then

	GENOME=/Main/MutantHuntWGS/Genomes/M_musculus/genome \
	GENOME_FASTA=/Main/MutantHuntWGS/Genomes/M_musculus/genome.fa \
	SIFT_GENOME=/Main/GRCm38.83
...
then
	...
	do
		...
		then
			...
			java -Xmx4G -jar /Main/snpEff/snpEff.jar ann -v "$GENOME" "$VCF_FILE" > "$OUTPUT_FILE"/SNPeff_Output/"$VCF_NAME"/SNPeff_Annotations.vcf
	...
	do
		...
		then
			...
			##Run SIFT
			java -Xmx4G -jar /Main/SIFT4G_Annotator.jar -c -i "$VCF_FILE"  -d "$SIFT_GENOME" -r "$OUTPUT_FILE"/SIFT_Output/"$VCF_NAME"
